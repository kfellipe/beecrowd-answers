...
As mensagens devem ser impressas conforme a descrição do problema. Não esqueça de imprimir o enter após o final de cada linha,
caso contrário obterá "Presentation Error".

CÓDIGO FONTE
"""
notas = input().split(" ")
notas = [float(notas[0]),float(notas[1]),float(notas[2]),float(notas[3])]
pesos = [2,3,4,1]
...
for nota in notas:
    numerador += nota * pesos[index]
    denominador += pesos[index]
    index+=1
